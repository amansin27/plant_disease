...
def extract_features(image):
    # CRITICAL FIX: Add color_mode='rgb' to force a 3-channel image load.
    image = tf.keras.utils.load_img(
        image,
        target_size=(160, 160), # Check if this should be (161, 161) based on error,
        color_mode='rgb'       # <-- FIX
    )
    feature = tf.keras.utils.img_to_array(image)
    
    # You might also want to normalize the feature array here if your model expects it
